...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── 2. Push files to GitHub (empty repo — no base tree) ──
    await new Promise(r => setTimeout(r, 1000)) // let GitHub provision
    ...
    const treeItems = await Promise.all(
      codeFiles.map(async (f) => {
        const blob = await githubRequest(`/repos/${GITHUB_ORG}/${repoName}/git/blobs`, 'POST', {
